##import packages needed in this exercise. zhenglin hu
...
resourcegroup_name = 'lhu'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'lhu'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
# Here, we add an entry for first car with five pieces of data - the make, model, year, color, and the price
#
...
print('Created entry for a Latte...\n')
time.sleep(1)
...
coffee.cupprice = 5.3
table_service.insert_entity('itemstable', coffee)
print('Created entry for Mocha...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeshop'
coffee.RowKey = '006'
coffee.sku = 'BLK203943'
coffee.brand = 'Starbucks'
coffee.flavor = 'Espresso'
coffee.cupsize = 10
coffee.cupprice = 4.3
table_service.insert_entity('itemstable', coffee)
print('Created entry for Espresso...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cardealership'", select='model,make, year,color,price')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeshop'", select='brand,flavor,cupsize,cupprice')
for item in items:
    print('Brand: ' + item.brand)
    print('Flavor: ' + item.flavor)
    print('Size of the Cup: ' + item.cupsize)
    print('Price per Cup: ' + str(item.cupprice) + '\n')
